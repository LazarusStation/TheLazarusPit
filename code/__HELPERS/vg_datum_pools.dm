...
/var/global/list/pooledvariables_vg = new
...
/datum/proc/createVariables()
	pooledvariables_vg[type] = new/list()
	var/list/exclude = global.exclude + args

	for(var/key in vars)
		if(key in exclude)
			continue
		pooledvariables_vg[type][key] = initial(vars[key])
...
/datum/proc/resetVariables()
	if(!pooledvariables_vg[type])
		createVariables(args)

	for(var/key in pooledvariables_vg[type])
		vars[key] = pooledvariables_vg[type][key]
...
/client/proc/debug_pooling()
	...
	for(var/key in pooledvariables_vg[type])
		if(pooledvariables_vg[type][key])
			L += "<br>[key] = [pooledvariables_vg[type][key]]"
		else
			L += "<br>[key] = null"
